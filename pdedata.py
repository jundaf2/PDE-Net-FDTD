...
class PDESolver(object):
    ...
    def predict(self, u1, u0, j2, j0, t):
        ...
        # print(N)
        for i in range(0, N):
            j2set.append(real(j2))
            j0set.append(real(j0))
            u1, u0, j2, j0 = self.step(u1, u0, j2, j0)
        return u1, u0, np.array(j2set), np.array(j0set)
...
class _fdm2d(PDESolver):
    ...
    def initgen(self):
        assert hasattr(self, 'dt')
        dist = 0
        object_locationx = self.mesh_size[0] // 2 #+ random.randint(-self.mesh_size[0] // 4, self.mesh_size[1] // 4)
        object_locationy = self.mesh_size[1] // 2 #+ random.randint(-self.mesh_size[0] // 4, self.mesh_size[1] // 4)
        object_radius = random.randint(2, self.mesh_size[0] // 4)
        dist = 0

        while dist <= object_radius+1: # ensure src location is not inside the object
            self.src_locx = self.mesh_size[0] // 2 + random.randint(-self.mesh_size[0] // 3, self.mesh_size[1] // 3)
            self.src_locy = self.mesh_size[1] // 2 + random.randint(-self.mesh_size[0] // 3, self.mesh_size[1] // 3)
            dist = np.sqrt((self.src_locx-object_locationx) ** 2 + (self.src_locy-object_locationy) ** 2)

        epsr = 1. * random.randint(2, 10)
        sigma = 1e9  # random.randint(1,10)
...
class TorchPDEDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample['u0'] = u0
        sample['u1'] = u1
        sample['epsr'] = espr #self.pde.epsr
        sample['sigma'] = sigma #self.pde.sigma
        if isinstance(self.T, float): #未来某个时间
            ut1, ut0, jt2, jt0 = fdm2d.predict(u1, u0, j2, j0, self.T)
        else: #未来多个时间
